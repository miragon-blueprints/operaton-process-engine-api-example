...
The backend is a Spring Boot 4 app, and Spring's Maven plugin can build an OCI image directly from the
...
We produce an **OCI image for the backend with Spring's `spring-boot:build-image`** (buildpacks, no
Dockerfile).

- **Backend image** — `./mvnw -DskipTests install && ./mvnw -pl service/app spring-boot:build-image
  -DskipTests` builds `operaton-process-engine-api-example/app:<version>` (the `<image><name>` block
  of `service/app/pom.xml`, JVM pinned via `BP_JVM_VERSION=21`). Buildpacks give a layered,
...
- **Positive:** `spring-boot:build-image` produces a runnable backend container with no Dockerfile to
  maintain. The build & deployment dimension is now filled for a headless service.
